            ...
            if not paused:
               ...
               #산성비 구현
                if  (int(pm_pattern1_count % 20) == 0):
                    ...
                    pm.put_img("./sprites/water_drop.png")
                    pm.change_size(40,40)
                    pm.x = random.randrange(playerDino.rect.left, playerDino.rect.right)
                    pm.y = 10
                    pm.move = 3
                    pm_list.append(pm)
                pm_pattern1_count += 1
...
def gameplay_story5():
    ...
    while not gameQuit:
        while startMenu:
            pass
        while not gameOver and pking.hp>0:
            if pygame.display.get_surface() == None:
                print("Couldn't load display surface")
                gameQuit = True
                gameOver = True
            ...
            if not paused:
                ...
                # 보스 몬스터 패턴0(위에서 가만히 있는 패턴): 보스 익룡이 쏘는 미사일.
                if (isPkingTime) and (pking.pattern_idx == 4) and (int(pm_pattern4_count % 20) == 0):
                    pm=obj()
                    pm.put_img("./sprites/pking bullet.png")
                    pm.change_size(15,15)
                    pm.x = round(pking.rect.centerx)
